...
export const LeadConciergeDemo = forwardRef<DemoActions>((props, ref) => {
  ...
  const handleSendMessage = useCallback(async (content: string, type: 'sms' | 'email' = 'sms') => {
    ...
    // Don't use AI API - all flows are now structured and handled above
    // This section should not be reached in normal flow
    return;

    // Check if user is asking about a property - recommend matching properties from cards
    const propertyKeywords = ['property', 'house', 'home', 'apartment', 'bedroom', 'bed', 'bathroom', 'bath'];
    const isAskingAboutProperty = propertyKeywords.some(keyword => lowerContent.includes(keyword)) || 
                                   lowerContent.match(/\d+\s*(bed|bath|bedroom|bathroom)/) ||
                                   lowerContent.match(/\$\d+/);
    
    if (isAskingAboutProperty && startedFromPropertyCard) {
      // Find matching properties from cards based on user request
      const matchingProperties = PROPERTIES.filter(prop => {
        const bedsMatch = lowerContent.match(/(\d+)\s*bed/);
        const priceMatch = lowerContent.match(/\$?(\d+)/);
        const beds = bedsMatch ? parseInt(bedsMatch[1]) : null;
        const price = priceMatch ? parseInt(priceMatch[1]) * 1000 : null;
        
        if (beds && prop.beds !== beds) return false;
        if (price && Math.abs(prop.price - price) > 200000) return false; // Within $200k
        return true;
      });

      if (matchingProperties.length > 0) {
        setIsTyping(true);
        const recommended = matchingProperties[0];
        const recommendedIndex = PROPERTIES.findIndex(p => p.address === recommended.address);
        if (recommendedIndex >= 0) {
          setSelectedPropertyIndex(recommendedIndex);
        }
        
        const responseMessage: Message = {
          role: 'agent',
          content: type === 'sms'
            ? `Great! I have ${recommended.address} available. It's ${recommended.beds} bed, ${recommended.baths} bath, ${recommended.sqft.toLocaleString()} sqft for $${recommended.price.toLocaleString()}. Would you like to know more about this property?`
            : `Great! I have ${recommended.address} available.\n\nIt's ${recommended.beds} bed, ${recommended.baths} bath, ${recommended.sqft.toLocaleString()} sqft for $${recommended.price.toLocaleString()}.\n\nWould you like to know more about this property?\n\nBest regards,\nSarah Martinez`,
          timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
          isAuto: true,
          type,
        };
    ...
    try {
      // Get conversation history from current messages (using ref for latest state)
      ...
      // Call AI API - include available properties info for recommendations
      const response = await fetch('/api/ai/chat', {
        ...
        body: JSON.stringify({
          ...
          property: customPropertyRequirements && customPropertyRequirements !== 'awaiting' ? null : PROPERTIES[selectedPropertyIndex],
          type,
        }),
      });

      const data = await response.json();
      const aiResponse = data.content || "I'd love to help! Could you tell me more about what you're looking for?";
      ...
      // Check if response indicates conversation should end (we'll reach out)
      if (aiResponse.toLowerCase().includes("we'll reach out") || aiResponse.toLowerCase().includes("we will reach out")) {
        setQualification(prev => ({
          ...prev,
          nextAction: 'We will reach out',
        }));
        setCrmStatus('sent');
      }
      ...
      setFollowUpScheduled(prev => prev || true);
      setIsTyping(false);
    } catch (error) {
      console.error('Error getting AI response:', error);
      // Fallback response
      const fallbackResponse = type === 'sms'
